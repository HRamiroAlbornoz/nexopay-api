...
import { getSharedExpensesByWalletId } from '../../queries/shared-expense.queries';
...
export async function getExpenses(req: Request, res: Response, next: NextFunction): Promise<void> {
  try {
    ...
    const expenses = await getSharedExpensesByWalletId(wallet.id, limit, offset);

    res.status(200).json({ expenses, page, limit });
  } catch (err) {
    next(err);
  }
}
